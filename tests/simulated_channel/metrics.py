...
def check(name, raw, metrics):
    ...
    conditions={
        ...
        "no_nonce_reuse":(raw.get("reused_keystream_after_default_counter_restart",1)==0,"Restarting with the default unchanged key/counter repeats the CTR keystream."),
        "replays_rejected":(raw.get("replay_deliveries",1)==0,"Replayed valid framed messages reach the deframer output."),
        "rotation_supported":(raw.get("coordinated_key_rotation_api",0)==1,"No coordinated mid-stream key-rotation/retirement protocol is exposed."),
