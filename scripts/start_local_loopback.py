...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Start the WBHF local loopback node.")
...
def main() -> int:
    args = parse_args()
    binary = args.build_dir / "wbhf_local_node"
    if not binary.exists():
        raise RuntimeError(f"{binary} does not exist; build it with: cmake --build {args.build_dir} --target wbhf_local_node")
    command = [
        str(binary),
        "--receiver",
        args.receiver,
